...
The decision variables in a GP must have strictly positive values (that is, they can't be zero). This, together with subtraction not being allowed, means that GPs deal entirely with quantities greater than zero. This is a good fit for engineering design equations (which are often constructed to have only positive quantities), but any model with variables of unknown sign (such as forces and velocities without a predefined direction) may be quite difficult to express as a GP. Such models might be better expressed as :ref:`Signomials <signomials>`.
...
A Signomial Program has signomial constraints, and while they cannot be solved as quickly or to global optima, because they build on the structure of a GP they can be solved more quickly than a generic nonlinear program. More information can be found under :ref:`signomialprogramming`.
