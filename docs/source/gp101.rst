...
A Geometric Program (GP) is a type constrained non-linear optimization problem.

A GP consists of an objective and set of inequalities of a particular form, which define the solution for a set of variables which have strictly positive values (that is, they can't be zero).
